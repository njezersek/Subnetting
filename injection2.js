function calc(ip, maska){
  ...
  var maskaInt = ipToInt(maska);

  //izracunaj omrezje = ip AND maska
  var omrezjeInt = ipInt & maskaInt;
  //izracunaj brodcast = ip OR NEG maska
  var brodcastInt = ipInt | ~maskaInt;

  var razred = "";
  var maskaRazreda = "0.0.0.0";
  ...
  var stOmrezij = 2**countOnes(ipToBinary(intToIp(ipToInt(maskaRazreda) ^ ipToInt(maska))));
  ...
  for(var i=0; i<output.length; i++){
    textOut += output[i].vrednost + output[i].decimalna + " | " + output[i].binarna + "\n";
  }
...
function narediMasko(n){
  n = n.slice(1);
  n = Number(n);
  var maska = ("1".repeat(n) + "0".repeat(32-n));
